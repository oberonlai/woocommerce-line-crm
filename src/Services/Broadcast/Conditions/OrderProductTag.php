 ...
 * 買過商品標籤篩選條件
 *
 * 根據訂單項目中的商品標籤篩選用戶，支援 contains 和 contains_all 操作符.
 * 透過 product_tag taxonomy 關聯商品與標籤進行查詢.
 *
 * 效能優化重點：
 ...
 * 買過商品標籤篩選條件類別
 */
class OrderProductTag implements FilterConditionInterface {
	...
	public function build_sql( array $condition, object $wpdb, array &$joins ): ?string {
		...
		// 統一轉為陣列處理.
		$tag_ids = is_array( $value ) ? $value : array( $value );

		if ( 'contains' === $operator ) {
			return $this->build_contains_sql( $tag_ids, $wpdb );
		}

		if ( 'contains_all' === $operator ) {
			return $this->build_contains_all_sql( $tag_ids, $wpdb );
		}

		if ( 'not_contain' === $operator ) {
			return $this->build_not_contain_sql( $tag_ids, $wpdb );
		}

		if ( 'not_contain_all' === $operator ) {
			return $this->build_not_contain_all_sql( $tag_ids, $wpdb );
		}
	...
	/**
	 * 建構 contains SQL（買過任一標籤的商品，可跨訂單）
	 *
	 * 透過商品標籤關聯查詢訂單項目.
	 *
	 * 效能優化：
	 ...
	 * - 保留 HPOS 和傳統模式相容性.
	 *
	 * @param array  $tag_ids 標籤 ID 陣列.
	 * @param object $wpdb    WordPress 資料庫物件.
	 * @return string
	 */
	private function build_contains_sql( array $tag_ids, object $wpdb ): string {
		...
		$subquery = "
			...
			INNER JOIN {$wpdb->term_relationships} tr
				ON (oim.meta_value = tr.object_id AND oim.meta_key = '_product_id')
				OR (oim.meta_value = tr.object_id AND oim.meta_key = '_variation_id')
		...
		return $wpdb->prepare(
			...
			array_merge( $tag_ids, $valid_statuses, $valid_statuses )
		);
	}

	/**
	 * 建構 contains_all SQL（同一訂單內包含所有標籤的商品）
	 *
	 * 透過商品標籤關聯查詢訂單項目.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_contains_all_sql( array $tag_ids, object $wpdb ): string {
		$tag_placeholders = implode( ', ', array_fill( 0, count( $tag_ids ), '%d' ) );
		$required_count   = count( $tag_ids );
		...
		$subquery = "
			...
			FROM (
				SELECT
					oi.order_id,
					COALESCE(wco.customer_id, pm_customer.meta_value) as customer_id,
					COUNT(DISTINCT tt.term_id) as matched_count
				FROM {$wpdb->prefix}woocommerce_order_items oi
				INNER JOIN {$wpdb->prefix}woocommerce_order_itemmeta oim
					ON oi.order_item_id = oim.order_item_id
				INNER JOIN {$wpdb->term_relationships} tr
		...
		return $wpdb->prepare(
			"u.wp_user_id IN ($subquery)",
			array_merge( $tag_ids, $valid_statuses, $valid_statuses, array( $required_count ) )
		);
	}
	...
	/**
	 * 建構 not_contain SQL（完全沒買過任一標籤的商品）
	 *
	 * 透過商品標籤關聯查詢訂單項目.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_not_contain_sql( array $tag_ids, object $wpdb ): string {
		$tag_placeholders = implode( ', ', array_fill( 0, count( $tag_ids ), '%d' ) );
	...
	/**
	 * 建構 not_contain_all SQL（沒有在同訂單買齊所有標籤的商品）
	 *
	 * 透過商品標籤關聯查詢訂單項目.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_not_contain_all_sql( array $tag_ids, object $wpdb ): string {
		$tag_placeholders = implode( ', ', array_fill( 0, count( $tag_ids ), '%d' ) );
		$required_count   = count( $tag_ids );
	...
	public function get_ui_config(): array {
		return array(
			'type'            => 'order_product_tag',
			'label'           => __( '購買商品標籤條件', 'otz' ),
			'icon'            => 'dashicons-tag',
			'operators'       => array(
				'contains'        => array(
					'label'       => __( '買過任一標籤', 'otz' ),
					'description' => __( '客戶買過任一指定標籤的商品（可跨訂單）', 'otz' ),
				),
				'contains_all'    => array(
					'label'       => __( '買齊所有標籤', 'otz' ),
					'description' => __( '客戶在同一訂單內買過所有指定標籤的商品', 'otz' ),
				),
				'not_contain'     => array(
					'label'       => __( '未買過任一標籤', 'otz' ),
					'description' => __( '客戶從未買過任一指定標籤的商品', 'otz' ),
				),
				'not_contain_all' => array(
					'label'       => __( '未買齊所有標籤', 'otz' ),
					'description' => __( '客戶沒有在同一訂單內買齊所有指定標籤的商品（可能買過部分或完全沒買）', 'otz' ),
				),
			),
			'value_component' => 'ProductTagSelector',
			'value_config'    => array(
				'type'        => 'ajax_select',
				'multiple'    => true,
				'ajax_action' => 'otz_search_product_tags',
				'placeholder' => __( '搜尋商品標籤...', 'otz' ),
				'min_input'   => 0,
			),
		);
	}
