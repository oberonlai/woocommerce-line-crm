 ...
 * 買過商品分類篩選條件
 *
 * 根據訂單項目所屬的商品分類篩選用戶，支援 contains、contains_all、not_contain、not_contain_all 操作符.
 * 透過 term_relationships 關聯商品與分類，同時支援簡單商品和可變商品規格.
 *
 * 效能優化重點：
 ...
 * 買過商品分類篩選條件類別
 */
class OrderProductCategory implements FilterConditionInterface {
	...
	public function build_sql( array $condition, object $wpdb, array &$joins ): ?string {
		...
		// 統一轉為陣列處理.
		$category_ids = is_array( $value ) ? $value : array( $value );

		if ( 'contains' === $operator ) {
			return $this->build_contains_sql( $category_ids, $wpdb );
		}

		if ( 'contains_all' === $operator ) {
			return $this->build_contains_all_sql( $category_ids, $wpdb );
		}

		if ( 'not_contain' === $operator ) {
			return $this->build_not_contain_sql( $category_ids, $wpdb );
		}

		if ( 'not_contain_all' === $operator ) {
			return $this->build_not_contain_all_sql( $category_ids, $wpdb );
		}
	...
	/**
	 * 建構 contains SQL（買過任一分類下的商品，可跨訂單）
	 *
	 * 支援簡單商品和可變商品規格的查詢.
	 *
	 * 效能優化：
	 ...
	 * - 保留 HPOS 和傳統模式相容性.
	 *
	 * @param array  $category_ids 分類 ID 陣列.
	 * @param object $wpdb         WordPress 資料庫物件.
	 * @return string
	 */
	private function build_contains_sql( array $category_ids, object $wpdb ): string {
		...
		return $wpdb->prepare(
			...
			array_merge( $category_ids, $valid_statuses, $valid_statuses )
		);
	}

	/**
	 * 建構 contains_all SQL（同一訂單內包含所有分類的商品）
	 *
	 * 支援簡單商品和可變商品規格的查詢.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_contains_all_sql( array $category_ids, object $wpdb ): string {
		$category_placeholders = implode( ', ', array_fill( 0, count( $category_ids ), '%d' ) );
		$required_count        = count( $category_ids );
		...
		return $wpdb->prepare(
			"u.wp_user_id IN ($subquery)",
			array_merge( $category_ids, $valid_statuses, $valid_statuses, array( $required_count ) )
		);
	}
	...
	public function validate( array $condition ): bool {
		...
		// contains 和 not_contain: 支援整數或陣列.
	...
	/**
	 * 建構 not_contain SQL（完全沒買過任一分類的商品）
	 *
	 * 支援簡單商品和可變商品規格的查詢.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_not_contain_sql( array $category_ids, object $wpdb ): string {
		$category_placeholders = implode( ', ', array_fill( 0, count( $category_ids ), '%d' ) );
	...
	/**
	 * 建構 not_contain_all SQL（沒有在同訂單買齊所有分類的商品）
	 *
	 * 支援簡單商品和可變商品規格的查詢.
	 *
	 * 效能優化：
	 ...
	 * @return string
	 */
	private function build_not_contain_all_sql( array $category_ids, object $wpdb ): string {
		$category_placeholders = implode( ', ', array_fill( 0, count( $category_ids ), '%d' ) );
		$required_count        = count( $category_ids );
	...
	public function get_ui_config(): array {
		return array(
			...
			'label'           => __( '購買商品分類條件', 'otz' ),
			'icon'            => 'dashicons-category',
			'operators'       => array(
				'contains'        => array(
					'label'       => __( '買過任一分類', 'otz' ),
					'description' => __( '客戶買過任一指定分類下的商品（可跨訂單）', 'otz' ),
				),
				'contains_all'    => array(
					'label'       => __( '買齊所有分類', 'otz' ),
					'description' => __( '客戶在同一訂單內買過所有指定分類下的商品', 'otz' ),
				),
				'not_contain'     => array(
					'label'       => __( '未買過任一分類', 'otz' ),
					'description' => __( '客戶從未買過任一指定分類下的商品', 'otz' ),
				),
				'not_contain_all' => array(
					'label'       => __( '未買齊所有分類', 'otz' ),
					'description' => __( '客戶沒有在同一訂單內買齊所有指定分類下的商品（可能買過部分或完全沒買）', 'otz' ),
				),
			),
			'value_component' => 'CategorySelector',
			'value_config'    => array(
				'type'        => 'ajax_select',
				'multiple'    => true,
				'ajax_action' => 'otz_search_product_categories',
				'placeholder' => __( '搜尋商品分類...', 'otz' ),
				'min_input'   => 0,
			),
		);
	}
