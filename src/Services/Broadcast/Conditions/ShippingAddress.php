 ...
 * 運送地址篩選條件
 *
 * 根據用戶個人資料的運送地址欄位進行模糊比對篩選,支援單一關鍵字搜尋多個地址欄位.
 *
 * 搜尋欄位包含 (wp_usermeta):
 * - shipping_country (國家)
 ...
 *
 * 效能優化重點:
 * 1. 直接查詢用戶資料:從 wp_usermeta 表查詢用戶的運送地址.
 ...
 * 運送地址篩選條件類別
 */
class ShippingAddress implements FilterConditionInterface {
	...
	/**
	 * 建構 contains SQL(運送地址包含關鍵字)
	 *
	 * 從 wp_usermeta 表查詢用戶的運送地址,只要任一欄位符合即算中.
	 *
	 * 查詢邏輯:
	 * - 搜尋 5 個運送地址相關欄位.
	 * - 使用 LIKE 模糊比對.
	...
	private function build_contains_sql( string $keyword, object $wpdb ): string {
		...
		// 從 wp_usermeta 查詢運送地址.
		$subquery = "
			SELECT DISTINCT user_id
			FROM {$wpdb->usermeta}
			WHERE meta_key IN ('shipping_country', 'shipping_state', 'shipping_city', 'shipping_address_1', 'shipping_address_2')
	...
	/**
	 * 建構 not_contain SQL(運送地址不包含關鍵字)
	 *
	 * 從 wp_usermeta 表查詢用戶的運送地址,所有欄位都不符合才算中.
	 *
	 * 查詢邏輯:
	 * - 搜尋 5 個運送地址相關欄位.
	 * - 使用 LIKE 模糊比對.
	...
	public function get_ui_config(): array {
		return array(
			...
			'operators'       => array(
				'contains'    => array(
					'label'       => __( '包含', 'otz' ),
					'description' => __( '運送地址包含指定關鍵字(搜尋國家、州/省/縣市、城市、地址)', 'otz' ),
				),
				'not_contain' => array(
					'label'       => __( '不包含', 'otz' ),
					'description' => __( '運送地址不包含指定關鍵字', 'otz' ),
				),
			),
